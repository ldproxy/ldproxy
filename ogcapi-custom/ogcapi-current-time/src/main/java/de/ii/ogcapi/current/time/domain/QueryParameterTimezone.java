 ...
 */
package de.ii.ogcapi.current.time.domain;
...
@Singleton
@AutoBind
public class QueryParameterTimezone extends OgcApiQueryParameterBase
    implements TypedQueryParameter<ZoneId>{

  private Schema<?> schema = null;
  private final SchemaValidator schemaValidator;

  @Inject
  public QueryParameterTimezone(SchemaValidator schemaValidator) {
  ...
  public void applyTo(ContainerRequestContext requestContext, QueryParameterSet parameters) {
    if (parameters.getTypedValues().containsKey(getName())) {
      ZoneId timezone = (ZoneId) parameters.getTypedValues().get(getName());
      requestContext.setProperty("timezone", timezone);
    }
  }
