...
const getCollectionUrl = (baseUrl: URL): URL => {
...
const getCrsLabel = (crs: string): string => {
...
const CrsEditor = () => {
  ...
  const { obj: collection, isLoaded, error } = useApiInfo<CollectionCrs>(collectionUrl);
  ...
  useEffect(() => {
    ...
    const initialCrs = normalizeCrs(initialQuery.crs as string | undefined, defaultCollectionCrs);
  ...
  const onDraftChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
