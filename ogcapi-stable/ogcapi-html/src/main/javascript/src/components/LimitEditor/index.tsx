...
const LimitEditor = ({
  ...
  const normalizedOptions = useMemo(
    () =>
      [...new Set((limitOptions || []).map(parsePositiveInteger).filter((v): v is number => Number.isInteger(v)))].sort(
        (a, b) => a - b,
      ),
    [limitOptions],
  );
  ...
  const getSelectValueForLimit = (limit: number | null): string => {
    ...
    if (Number.isInteger(limit) && optionsWithDefaults.includes(limit as number)) {
      return String(limit);
    }
            ...
            <Col md="8" className="px-0 d-flex align-items-center">
              {optionsWithDefaults.length > 0 && (
                <Input
                  type="select"
                  bsSize="sm"
                  className="custom-select custom-select-sm d-inline-block mr-2"
                  style={{ width: "120px" }}
