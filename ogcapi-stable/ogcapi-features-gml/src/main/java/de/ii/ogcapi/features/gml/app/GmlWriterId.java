  ...
  public void onValue(EncodingAwareContextGml context, Consumer<EncodingAwareContextGml> next)
    ...
    if (context.schema().isPresent() && Objects.nonNull(context.value())) {
      ...
      if (currentSchema.isId()) {
        ...
        // versions-as-features-unique-ids, the default for GML on versioned collections); the
        // gml:identifier element must still carry the canonical feature id, which the
        // transformer stashes on context.canonicalFeatureId().
