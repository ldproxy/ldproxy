 ...
 *     uses "val-as-title" as the default, all other feature encodings use "val-as-code" as the
 *     default. Note: Explicit codelist transformations in the provider or in the service
 *     configuration are always executed, the "profile" parameter with a value "val-as-code" does
 *     not disable these transformations.
 * @scopeDe Wenn das Feature-Schema mindestens eine Eigenschaft mit einer "Codelist"-Einschränkung
 *     enthält, können zwei Profile verwendet werden, um die Darstellung von codierten Werten in der
 *     Antwort auszuwählen. Unterstützt werden "val-as-code" (der Code) und "val-as-title" (die mit
 *     dem Code verbundene Bezeichnung). HTML verwendet "val-as-title" als Default, alle anderen
 *     Formate "val-as-code". Hinweis: Explizite Codelisten-Transformationen im Provider oder in der
